...
class ExecuteT5Hidden(P.Pipeline[Tp.Optional[P.TextEntry], Tp.Optional[T5PredictionOutput]]):
    ...
    def __call__(self, entry: Tp.Optional[P.TextEntry]) -> Tp.Optional[T5PredictionOutput]:
        if entry is None: return None
        text_tokenized = self.tokenizer.batch_encode_plus((entry["text"],), padding=True, truncation=True, return_tensors="pt")
        ...
        hiddens = [h.detach().cpu() for h in result.huggingface.decoder_hidden_states[0]]

        return {"hidden": hiddens, "predict": logits, "input": entry}


class T5HiddenPredictToLogits(P.Pipeline[Tp.Optional[T5PredictionOutput], Tp.Optional[P.ArrayEntry]]):
    def __call__(self, prediction: Tp.Optional[T5PredictionOutput]) -> Tp.Optional[P.ArrayEntry]:
        if prediction is None: return None

        uid: str = prediction["input"]["uid"] if prediction["input"]["uid"] is not None else "No-UID-placeholder"
        return {
            "uid": uid,
...
class T5PredictToHidden(P.Pipeline[Tp.Optional[T5PredictionOutput], Tp.Optional[P.ArrayEntry]]):
    def __call__(self, prediction: Tp.Optional[T5PredictionOutput]) -> Tp.Optional[P.ArrayEntry]:
        if prediction is None: return None
...
if __name__ == "__main__":
    pipe = ExecuteT5Hidden(Path("./data/checkpoint/models-small.0621.hidden.pt")) >> T5HiddenPredictToLogits()
    result = pipe({
        "uid": "001",

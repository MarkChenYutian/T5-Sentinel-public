...
class ExecuteT5(P.Pipeline[Tp.Optional[P.TextEntry], Tp.Optional[T5PredictionOutput]]):
    ...
    def __call__(self, entry: Tp.Optional[P.TextEntry]) -> Tp.Optional[T5PredictionOutput]:
        ...
        logits = result.probabilities.cpu()
        hiddens = [h.cpu() for h in result.huggingface.decoder_hidden_states[0]]

        return {"hidden": hiddens, "predict": logits, "input": entry}


class T5PredictToLogits(P.Pipeline[Tp.Optional[T5PredictionOutput], Tp.Optional[P.ArrayEntry]]):
    def __call__(self, prediction: Tp.Optional[T5PredictionOutput]) -> Tp.Optional[P.ArrayEntry]:
        ...
        return {
            "uid": uid,
            "data": prediction["predict"][0].numpy(),
...
if __name__ == "__main__":
    pipe = ExecuteT5(Path("./data/checkpoint/models.t5_sentinel.0613.pt")) >> T5PredictToLogits()
    result = pipe({
        "uid": "001",

...
from evaluator.models.t5_sentinel.t5_pipeline import ExecuteT5, T5PredictToLogits, T5PredictToHidden
from pathlib import Path
from memoizer import memoize

CHECKPOINT = "t5_sentinel.0613.pt"
...
def evaluate_prediction_impl(from_file: Path) -> T.Sequence[P.ArrayEntry]:
    pipe = P.FromJsonStr() >> ExecuteT5(
        Path("./data/checkpoint", CHECKPOINT)) >> T5PredictToLogits() >> P.ToSingletonList(T.Optional[P.ArrayEntry])
...
def evaluate_hiddens_impl(from_file: Path) -> T.Sequence[P.ArrayEntry]:
    pipe = P.FromJsonStr() >> ExecuteT5(
        Path("./data/checkpoint", CHECKPOINT)) >> T5PredictToHidden() >> P.ToSingletonList(T.Optional[P.ArrayEntry])
...
def evaluate_removals_impl(from_file: Path, singleton: str) -> T.Sequence[P.ArrayEntry]:
    pipe = P.FromJsonStr() >> P.RemoveSingleton(singleton) >> ExecuteT5(
        Path("./data/checkpoint", CHECKPOINT)) >> T5PredictToLogits() >> P.ToSingletonList(T.Optional[P.ArrayEntry])
...
def evaluate_removals(from_files: T.Sequence[Path], singleton: str) -> T.Sequence[P.ArrayEntry]:
    result = []
    for from_file in from_files:

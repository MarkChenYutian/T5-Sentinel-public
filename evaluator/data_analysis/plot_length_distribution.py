...
def get_string_length(s: str): return len(s)
def get_token_length(s: str): return len(Tokenizer.encode(s))
def argeq(a, b): return a[0] == b[0]


@memoize(Path("cache", "dataset_str_length_cache.pt"), arg_eq=argeq)
def get_data_tok_length(dataset_name: Tp.Literal["human", "chatgpt", "palm", "llama", "gpt2"]):
    selected_files = {
...
def plot_data_length_distribution():
    ...
    for idx, (category, data, color, ax) in enumerate(zip(categories, all_data, colors, axes)):
...
if __name__ == "__main__":
    TASKS = [
        plot_data_length_distribution,
        plot_data_length_distribution_cut
    ]
    for task in TASKS:
